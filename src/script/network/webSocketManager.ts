...
export class WebSocketManager {
    ...
    constructor(url: string, token: string) {
        ...
        if (game.users.current?.isGM && game.users.current?.hasRole(CONST.USER_ROLES.GAMEMASTER)) {
            ...
            foundry.helpers.Hooks.on("userConnected", this.reevaluatePrimaryGM.bind(this));
            //DeprecatedHookConfig
    ...
    private checkIfPrimaryGM(): boolean {
        ...
        if (!game.users.current?.isGM && !game.users.current?.hasRole(CONST.USER_ROLES.GAMEMASTER)) return false;

        const currentUserId = game.user?.id;
        // Only consider active users with role 4 (full GM), not Assistant GMs (role 3)
        const activeGMs = game.users?.filter(u => u.hasRole(CONST.USER_ROLES.GAMEMASTER) && u.active) || [];
    ...
    connect(): void {
        ...
        try {
            ...
            if (game.world) {
                wsUrl.searchParams.set('worldId', game.world._source.id);
                wsUrl.searchParams.set('worldTitle', game.world._source.title);
            }

            // Add version and system information
            wsUrl.searchParams.set('foundryVersion', game.version);
            wsUrl.searchParams.set('systemId', game.system._source.id);
            wsUrl.searchParams.set('systemTitle', game.system._source.title || game.system._source.id);
            wsUrl.searchParams.set('systemVersion', (game.system as any).version || 'unknown');

            // Add custom name if set
            const customName = game.settings.get(moduleId, SETTINGS.CUSTOM_NAME) as string;
            if (customName) {
