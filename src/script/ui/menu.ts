...
import { moduleId, SETTINGS, SETTINGS_AI, SETTINGS_SYSTEM } from '../constants';
import { ModuleLogger } from '../utils/logger';

export class Popup_SETTING_AI extends foundry.applications.api.DialogV2 {
    ...
    override async _renderHTML(_context: any, _options: any) {
        ...
        for (const config of Object.values(SETTINGS_AI)) {
            ...
            input.readOnly = config.key === SETTINGS.AI_API_KEY ? false : true;
            input.type = config.key === SETTINGS.AI_API_KEY ? "password" : "text";
    ...
    /** @override */
    override async _onSubmit(target: HTMLButtonElement | null, event: PointerEvent | SubmitEvent) {
        event.preventDefault(); // Prevent default browser dialog dismiss behavior.
        event.stopPropagation();

        const action = target?.dataset.action ?? (event as any).submitter?.dataset.action;
        const activeForm = this.element?.querySelector<HTMLFormElement>("form");
        ...
        const prevApiKey = game.settings.get(moduleId, SETTINGS.AI_API_KEY);
        const nextApiKey = String(formData.get(SETTINGS_SYSTEM[SETTINGS.AI_API_KEY].name) ?? "");
        const apiKeyChanged = nextApiKey !== prevApiKey;

        if (apiKeyChanged) {
            await game.settings.set(moduleId, SETTINGS.AI_API_KEY, nextApiKey);
        }
...
export class Popup_SETTING_INFO extends foundry.applications.api.DialogV2 {
    ...
    override async _renderHTML(_context: any, _options: any) {
        ...
        for (const config of Object.values(SETTINGS_SYSTEM)) {
            ...
            if (config.key !== SETTINGS.API_KEY) {
